...
#       - If `tags.gpu` is missing, GPU (and optionally TP/PP) may be inferred from workspace URIs:
...
#   5. Display names:
#       - Base name formatting: if the model name begins with 2+ short alphabetic tokens (<=4 chars) separated by hyphens,
#         render them as an uppercase hyphenated prefix (e.g., "gpt-oss-20b" -> "GPT-OSS 20B ..."). Otherwise, use Title Case
#         with spaces and convert size suffix like "20b" -> "20B" (e.g., "llama-3.1-instruct" -> "Llama 3.1 Instruct ...").
#       - Public: "<Model Name> <GPU>x<count> [SM<sm>] [V<v>] [ONNX] <PRECISION> <Profile>"
#         - If GPU/count were inferred from URI, they are reflected in the display name without mutating tags.
...
#       - URI inference:
#           a) When GPU is inferred, add spec.GPU and spec.COUNT (COUNT uses inferred TP*PP when available; otherwise from tags).
#           b) When TP/PP are explicitly present in the URI and missing in tags, add spec.TP/spec.PP (do not add to tags).
#       - NIM VERSION and DOWNLOAD SIZE are included in spec; DOWNLOAD SIZE may be "UNKNOWN" depending on NGC CLI response.
...
#   9. Examples:
#       - Missing GPU in tags; URI: "...:a10gx1-trt-fp16-....":
#           spec.GPU=A10G, spec.COUNT=1; displayName includes "A10Gx1"; TP/PP omitted unless explicitly present in URI.
#       - Hybrid URI: "...:rtx6000-blackwell-svx1-trt-fp16-....":
#           spec.GPU=RTX6000-BLACKWELL, spec.COUNT=1; displayName includes "RTX6000-BLACKWELLx1".
#       - Hyphenated acronym model: "gpt-oss-20b":
#           base renders as "GPT-OSS 20B"; e.g., "GPT-OSS 20B A10Gx4 MXFP4".
####################################################################################################################
...
def main():
    ...
    parser.add_argument("--no-include-onnx", dest="include_onnx", action="store_false", help="Exclude ONNX profiles.")
    parser.set_defaults(include_onnx=True)
    ...
    for profile in profiles:
        ...
        if gpu_tag:
            if should_ignore_profile(tags, args.whitelisted_gpus, args.platform, args.a100_max_count,
        ...
        for profile in profiles:
            ...
            if (
                ...
                if result and result[0]:
                    ...
                    print(f"Added generic SGLang profile {profile.get('id', 'unknown')} without GPU details for public platform")
                continue

            # Handle generic TRT-LLM inclusion (no GPU synthesis, include as-is)
            if is_generic_profile(tags):
                if "nim_workspace_hash_v1" in tags and isinstance(tags["nim_workspace_hash_v1"], str):
                    tags["nim_workspace_hash_v1"] = PlainScalarString(tags["nim_workspace_hash_v1"])
                result = profile_id_from_workspace(profile, "", return_base_uri=True)  # No specific GPU
                if result and result[0]:
                    ...
                    print(f"Added generic TRT-LLM profile {profile.get('id', 'unknown')} without GPU details for public platform")
